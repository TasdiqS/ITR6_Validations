...
@Entity
@Table(name = "newValid1") @SequenceGenerator(name="seq", initialValue=1, allocationSize=100)
public class ITR6Validation {	 
	...
	@GeneratedValue(strategy=GenerationType.SEQUENCE, generator="seq")@Id
	 ...
	 @Pattern(regexp = "[2-9]{1}[0-9]{11}")//must not start with 0 or 1 and should be 12 digits
	 private String AdhaarId;
	 
	 @Pattern(regexp = "[A-Z]{5}[0-9]{4}[A-Z]{1}")//first five char 4numbers and last 1 char
	 private String PanId;
	 
	 @Pattern(regexp = "[7-9]{1}[0-9]{9}")//start with 7,8 or 9 and 10 digits long
	 private String PhNo;
	 
	 @Pattern(regexp="[1-9]{1}[0-9]{5}")//Should be 6 digits and not start with 0
...
public ITR6Validation(int c_Id, @Pattern(regexp = "[a-zA-Z0-9& ]{1,30}") String c_name,
		@Pattern(regexp = "[a-zA-Z0-9&@#() ]{1,70}") String address,
		@Pattern(regexp = "[2-9]{1}[0-9]{11}") String adhaarId,
		@Pattern(regexp = "[A-Z]{5}[0-9]{4}[A-Z]{1}") String panId, @Pattern(regexp = "[7-9]{1}[0-9]{9}") String phNo,
		@Pattern(regexp = "[1-9]{1}[0-9]{5}") String zipCode,
